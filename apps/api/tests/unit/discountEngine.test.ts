...
it('caps fixed discount at subtotal', () => {
  const result = calculateDiscount(100, { type: 'FIXED', value: 150 }, 'cashier', ['orders_discount'])
  expect(result.discountAmount).toBeCloseTo(100, 2)
  expect(result.finalAmount).toBeCloseTo(0, 2)
})
...
it('caps discount at subtotal to prevent negative total', () => {
  const result = calculateDiscount(100, { type: 'FIXED', value: 200 }, 'cashier', ['orders_discount'])
  expect(result.discountAmount).toBeCloseTo(100, 2)
  expect(result.finalAmount).toBeCloseTo(0, 2)
})