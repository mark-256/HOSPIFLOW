...
- **RPO (Recovery Point Objective):** 24 hours minimum (daily backups by default)
- **RTO (Recovery Time Objective):** Depends on database size. Typical restore time for a 10 GB database is 5–15 minutes using `gunzip | psql`. Verify with actual restore tests.
